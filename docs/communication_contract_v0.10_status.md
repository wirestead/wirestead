...
## Stream-client accounting follow-up
...
termination and gather-prefix attribution. UDP/server targets still report no
...
| Statistics | [RuntimeStats](../wirestead/wrapper/runtime_stats.hpp), [counter implementation](../wirestead/diagnostics/runtime_stats_counter.hpp); each native cleanup/write-completion path | Legacy counter tests plus test_send_accounting.cc and test_tcp_send_accounting.cc and transport_stream_send_accounting.cc; discard/abort accounting is established for TCP/UDS clients and Serial |
...
| C-3.8-1 | Partial: TCP/UDS clients and Serial implemented; Decided gap remains on UDP/servers | Optional SendAccounting on TCP/UDS clients and Serial distinguishes stages/causes; UDP and server aggregation remain unsupported |
...
| C-5.5-2 | Partial: TCP/UDS clients and Serial now have a separate post-admission ledger | Legacy failed_sends semantics remain; UDP/server targets still lack logical-request accounting |
...
| --- | --- |
| 1: accepted versus delivered | Implemented result naming and docs; no delivery guarantee. Post-acceptance classification now exists for TCP/UDS clients and Serial |
...
| 6.1: cause-separated cleanup statistics | TCP/UDS clients and Serial distinguish explicit stop, connection loss and queue pressure in the new ledger. Other targets and UDP expiry remain missing |
...
1. **Post-acceptance accounting (required Decided gap):** extend the implemented
   stream-client ledger to UDP and server aggregation; track
...
for a concrete design and acceptance scenarios. TCP/UDS/Serial accounting is now implemented as documented above; the remaining
