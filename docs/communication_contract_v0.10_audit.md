...
| C-5.4-2 **Every** concurrent `stop()` caller returns after shutdown is complete | Decided | Client | **Implemented under D-1** (`TcpClientStopCompletionTest`). The code returned early from the `stop_requested_` / `started_` exchange, but measurement showed callers still waited in practice - through the io-thread join and the wrapper's own locking. The guarantee is now explicit rather than incidental: a callback-admission gate, and a completion signal from the teardown itself | `transport/tcp_client/tcp_client.cc:339-341`; `wrapper/tcp_client/tcp_client.cc:239-250` `[code]` | Differs | A second caller cannot treat the return as shutdown complete; it returns while the first caller is still tearing the object down |
| C-5.4-2 | Decided | Server | **Implemented under D-1** (`TcpServerStopCompletionTest`), same shape and same finding as the client row | `transport/tcp_server/tcp_server.cc:505-508`; `wrapper/tcp_server/tcp_server.cc:285-291` `[code]` | Differs | As above |
