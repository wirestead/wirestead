...
| C-5.4-2 **Every** concurrent `stop()` caller returns after shutdown is complete | Decided | Client | The first caller performs the shutdown; a concurrent second caller returns immediately from the `stop_requested_` / `started_` exchange, before the first has finished | `transport/tcp_client/tcp_client.cc:339-341`; `wrapper/tcp_client/tcp_client.cc:239-250` `[code]` | Differs | A thread that calls `stop()` second may destroy the object while the first caller is still tearing it down |
...
| C-5.1-2 A server session is a serial scope | Proposed | Server | Each session owns `net::strand<io_context::executor_type>`; the default server runs one io thread | `tcp_server_session.hpp:102`, `tcp_server_session.cc:33` `[code]` | Match | – |
...
| C-5.5-3 `stats()` is observational, fields not mutually consistent | Proposed | Client | A snapshot of independent atomics | `transport/tcp_client/tcp_client.cc:367-371` `[code]` | Match | – |
| C-5.5-3 | Proposed | Server | Server totals plus a per-session sum taken under `sessions_mutex_`, with peak queue depth taken as a max rather than a sum | `transport/tcp_server/tcp_server.cc:705-729` `[code]` | Match | – |

## 2. Acceptance, order, queue

| Rule ID | Contract status | Target | Observed implementation | Evidence | Verdict | User impact |
| --- | --- | --- | --- | --- | --- | --- |
| C-3.1-1 Stage 1 rejects empty and oversized payloads without waiting | Proposed | Client | Zero-length and above-maximum are rejected before any queue work | `transport/tcp_client/tcp_client.cc:386-399` `[code]` | Match | – |
...
| C-3.6-3 The result reports accepted and rejected counts per reason | Proposed | Server | Returns a single `bool`: true when at least one session accepted | `transport/tcp_server/tcp_server.cc:890-900` `[code]` | Differs | A caller cannot tell "all delivered" from "one of fifty delivered" |
...
| C-1-1 Shutdown complete covers outstanding internal work | Proposed | Server | `stop()` dispatches cleanup onto the io_context and waits **up to 2 seconds**; on timeout it runs cleanup directly on the calling thread | `transport/tcp_server/tcp_server.cc:544-560` `[code]` | Differs | If the executor is not being serviced, `stop()` returns after a bounded wait rather than when shutdown is complete, which is the case the contract's definition was written for |

## Cross-cutting observations

1. **The `bool` return decides several rows.** C-5.5-3, C-3.6-3, C-3.6-4 and
   C-3.8-1 all reduce to "the caller is told less than the contract assumes".
   The open decision on the result type (contract open item 1) settles them
   together.
