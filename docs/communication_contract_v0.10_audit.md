...
| C-5.4-2 **Every** concurrent `stop()` caller returns after shutdown is complete | Decided | Client | **Fixed under D-1** (`TcpClientStopCompletionTest`). Previously: the first caller performed the shutdown while a concurrent second caller returned immediately from the `stop_requested_` / `started_` exchange, before the first had finished | `transport/tcp_client/tcp_client.cc:339-341`; `wrapper/tcp_client/tcp_client.cc:239-250` `[code]` | Differs | A second caller cannot treat the return as shutdown complete; it returns while the first caller is still tearing the object down |
| C-5.4-2 | Decided | Server | **Fixed under D-1** (`TcpServerStopCompletionTest`). Previously the same shape: `stopping_.exchange(true)` returned early for the second caller | `transport/tcp_server/tcp_server.cc:505-508`; `wrapper/tcp_server/tcp_server.cc:285-291` `[code]` | Differs | As above |
