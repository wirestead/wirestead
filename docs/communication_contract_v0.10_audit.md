...
| C-5.4-2 **Every** concurrent `stop()` caller returns after shutdown is complete | Decided | Client | **Fixed under D-1** (`TcpClientStopCompletionTest`, `TcpStopAdmissionTest`). Measured on a clean checkout of the pre-D1 tree: with a callback running, the first `stop()` returned in 0 ms and the second after 400 ms, and on an externally run io_context `stop()` returned in 0 ms. Now every caller returns only once no callback of the object is running and the teardown itself has run | `transport/tcp_client/tcp_client.cc:339-341`; `wrapper/tcp_client/tcp_client.cc:239-250` `[code]` | Differs | A second caller cannot treat the return as shutdown complete; it returns while the first caller is still tearing the object down |
| C-5.4-2 | Decided | Server | **Fixed under D-1** (`TcpServerStopCompletionTest`), same shape as the client row; 2 of its 4 tests fail on the pre-D1 tree | `transport/tcp_server/tcp_server.cc:505-508`; `wrapper/tcp_server/tcp_server.cc:285-291` `[code]` | Differs | As above |
