...
// D-1 for the UDS server (docs/communication_contract_v0.10_decisions.md).
...
//
// Signals use bounded waits; CTest also bounds shutdown regressions.
...
class UdsServerStopCompletionTest : public ::testing::Test {
 protected:
  void SetUp() override { socket_path_ = TestUtils::makeUniqueUdsSocketPath("uds-d1").string(); }

  std::shared_ptr<wrapper::UdsClient> connect_client() {
    auto client = wirestead::uds_client(socket_path_).on_error([](auto&&) {}).build();
...
TEST_F(UdsServerStopCompletionTest, OutsideStopsWaitForASessionCallbackToFinish) {
  auto server = wirestead::uds_server(socket_path_).on_error([](auto&&) {}).build();
...
TEST_F(UdsServerStopCompletionTest, SessionCallbackRequestsStopWhileOutsideCallersWait) {
  auto server = wirestead::uds_server(socket_path_).on_error([](auto&&) {}).build();
...
TEST_F(UdsServerStopCompletionTest, RestartAfterStopAcceptsAndReceivesAgain) {
  auto server = wirestead::uds_server(socket_path_).on_error([](auto&&) {}).build();
  std::atomic<int> received{0};
  StopTestCleanup restart_cleanup{[&] {
    if (server) server->stop();
...
TEST_F(UdsServerStopCompletionTest, OutsideStopWaitsOnAnExternallyRunContext) {
  auto ioc = std::make_shared<boost::asio::io_context>();
  auto work = boost::asio::make_work_guard(*ioc);
  std::jthread ioc_thread([&] { ioc->run(); });

  auto server = std::make_shared<wrapper::UdsServer>(socket_path_, ioc);
  Signal in_callback, release_callback;
  std::atomic<bool> callback_finished{false};
...
// Cleanup must serialize with a session callback even when another I/O thread is free.
TEST_F(UdsServerStopCompletionTest, OutsideStopWaitsWithTwoExecutorThreads) {
  auto ioc = std::make_shared<boost::asio::io_context>();
  auto work = boost::asio::make_work_guard(*ioc);
  std::jthread ioc_thread([&] { ioc->run(); });
  std::jthread second_runner([&] { ioc->run(); });
