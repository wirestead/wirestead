  ...
  void setup_internal_handlers() {
    ...
    channel->on_bytes([this, generation, weak_impl, weak_alive](memory::ConstByteSpan data) {
      ...
      try {
        ...
        if (auto native = std::dynamic_pointer_cast<transport::Serial>(channel)) native->fail_receive();
    ...
    channel->on_state([this, generation, weak_impl, weak_alive](base::LinkState state) {
      ...
      detail::invoke_user_callback("serial", "on_connect", connected_handler, ConnectionContext(0));
      detail::invoke_user_callback("serial", "on_disconnect", lost_handler, ConnectionContext(0));
      ...
      if (error_lease.admitted() && terminal_handler)
        detail::invoke_user_callback("serial", "on_error", terminal_handler,
                                     channel_snapshot
...
ReceiveMemoryStats Serial::receive_stats() const {
