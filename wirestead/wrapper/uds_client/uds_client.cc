...
struct UdsClient::Impl : public std::enable_shared_from_this<Impl> {
  ...
  SendResult send_state(const std::shared_ptr<transport::UdsClient>& uds) {
    ...
    if (!uds) return SendResult::reject(SendRejection::NotReady);
  ...
  bool nonblocking_send(size_t size, bool best_effort_send, NativeWrite native_write, FallbackWrite fallback_write) {
    ...
    if (channel_ && !uds) return channel_->is_connected() && fallback_write(*channel_);
    const auto result = [&]() -> SendResult {
      auto validation = detail::validate_payload_size(size, uds ? uds->write_queue_limit() : std::nullopt);
      if (!validation.accepted()) return validation;
      const auto state = send_state(uds);
      if (!state.accepted()) return state;
      // The native call rechecks state and capacity together under its mutex.
      auto admitted = native_write(*uds);
  ...
  bool blocking_send(size_t size, NativeWrite native_write, FallbackWrite fallback_write) {
    ...
    bool custom;
    {
      ...
      custom = channel_ && !connection.uds;
    }
    if (custom) {
    ...
    const auto result = [&]() -> SendResult {
      {
        ...
        connection.uds = std::dynamic_pointer_cast<transport::UdsClient>(channel_);
        auto validation =
            detail::validate_payload_size(size, connection.uds ? connection.uds->write_queue_limit() : std::nullopt);
        if (!validation.accepted()) return validation;
        auto state = send_state(connection.uds);
        if (!state.accepted()) return state;
      ...
      for (int attempt = 0; attempt < kMaxBlockingSendAttempts; ++attempt) {
        ...
        bp_lock.unlock();
        std::shared_lock<std::shared_mutex> lock(mutex_);
        const auto state = send_state(connection.uds);
        if (!state.accepted()) return state;
